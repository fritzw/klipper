...
struct spidev_s {
    union {
        ...
        struct spi_software *spi_software_config;
    };
    struct gpio_out pin;
    uint8_t flags;
                ...
                , uint8_t data_len, uint8_t *data)
{
    if (CONFIG_HAVE_GPIO_BITBANGING && spi->flags & SF_SOFTWARE)
        spi_software_prepare(spi->spi_software_config);
    else
    ...
    if (CONFIG_HAVE_GPIO_BITBANGING && spi->flags & SF_SOFTWARE)
        spi_software_transfer(spi->spi_software_config, receive_data
                              , data_len, data);
    else
